...
def is_valid_nonempty_str(string: str, raise_if_fail=True) -> bool:
    '''Empty string check for use in endpoints & models (a bit trivial but helpful)'''
    if string == None or len(string) == 0:
        if raise_if_fail == True:
            raise f'Non-empty string expected'
        else:
            return False
    return True

def is_valid_date(date_or_string_or_num: datetime or str or int, raise_if_fail=True) -> bool:
...
def is_valid_country_code(country_code: str, raise_if_fail=True) -> bool:
